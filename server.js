...
app.post("/api/auth/login", checkBan, async (req, res) => {
  try {
    ...
    const publicUser = {
      ...
      // UUID aliases for compatibility
      uuid: user.public_id,
      publicId: user.public_id,
      public_id: user.public_id,

      // Keep internal ID available under a different name
      user_id: user.id,
    ...
    // JWT
    //
    // Keep numeric database ID
    // internally so existing backend
    // relationships continue working.
    //
    // Also include public UUID.
    // ----------------------------
    ...
    if (
      ...
      try {
        ...
        if (matches) {
          ...
          const s3Key =
            `profile-pics/${Date.now()}-${username}.jpg`;
...
app.get(
  ...
  async (req, res) => {
    try {
      // req.userId is the INTERNAL numeric ID
      const { rows } = await pool.query(
        `SELECT *
         FROM users
         WHERE id = $1`,
      ...
      return res.json({
        ...
        user: {
          ...
          // Keep internal DB ID separately
          user_id: user.id,
...
app.post("/api/users/me/follow-requests/:requestId/accept", authenticateToken, async (req, res) => {
  try {
    ...
    const followerId = request.rows[0].follower_id;
    ...
    try {
      await pool.query(
        `INSERT INTO notifications (user_id, from_user_id, type, reference_id, created_at)
         VALUES ($1, $2, 'follow_accepted', $3, NOW())`,
        [followerId, viewerId, requestId]
      );
    } catch (e) {
...
app.get("/api/notifications", authenticateToken, async (req, res) => {
  try {
    // Support both styles used by your authentication middleware.
    ...
    const notifications = rows.map((n) => {
      ...
      if (n.data) {
        try {
          ...
          console.warn(
            "[NOTIFICATIONS] Invalid JSON data:",
            n.id
          );
      ...
      const notification = {
        ...
        link: getNotificationLink(n.type, data),

        // Flatten notification data so the existing
        // frontend can continue using fields such as:
        // n.device, n.browser, n.reason, n.version, etc.
        ...data,
      };
    ...
    console.log(
      `[NOTIFICATIONS] User ${userId}: ${notifications.length} notifications, ${unreadCount} unread`
    );
    ...
    console.error(
      "[NOTIFICATIONS] Get notifications error:",
      err
    );
